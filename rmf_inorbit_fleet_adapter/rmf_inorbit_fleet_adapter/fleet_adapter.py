...
# https://github.com/open-rmf/fleet_adapter_template/blob/32f47a451ea67fa299a4fdb1a189d5db8df84b6b/fleet_adapter_template/fleet_adapter_template/fleet_adapter.py#
# The following is a summary of the introduced changes:
# - Adapted the configuration file usage
...
def initialize_fleet(config_yaml, api_key: str, nav_graph_path, node, use_sim_time, server_uri):
    # Profile and traits
        ...
        while len(missing_robots) > 0:
            ...
            for robot_name in list(missing_robots.keys()):
                ...
                robot = RobotAPI(
                    ...
                    max_lin_speed=fleet_config['limits']['linear'][0],
                    logger=node.get_logger(),
                    threshold_distance=fleet_config['fleet_handle']['threshold_distance'],
                    threshold_angle=fleet_config['fleet_handle']['threshold_angle'])

                node.get_logger().info(
                    f"Connecting to robot: {robot.name} with id {robot.robot_config['robot_id']}")
                position = robot.position()
                node.get_logger().info(f"got pose: {position}")
                if position is None:
                    continue
                if len(position) > 2:
                    node.get_logger().info(
                        f"Initializing robot: {robot.name} with id {robot.robot_config['robot_id']}")
                    ...
                    if (initial_waypoint is not None) and\
                        ...
                        node.get_logger().info(
                            ...
                            f"initialize starts for robot [{robot.name} with id {robot.robot_config['robot_id']}]")
                        ...
                        node.get_logger().info(
                            ...
                            f"{robot.name} with id {robot.robot_config['robot_id']}")
                    ...
                    if starts is None or len(starts) == 0:
                        node.get_logger().error(
                            f"Unable to determine StartSet for {robot.name} with id {robot.robot_config['robot_id']}")
                        continue
                    ...
                    if robotCommandHandle.initialized:
                        # Add robot to fleet
                        fleet_handle.add_robot(robotCommandHandle,
                                               robot.name,
                                               profile,
                                               [starts[0]],
                                               partial(_updater_inserter,
                                                       robotCommandHandle))
                        node.get_logger().info(
                            f"Successfully added new robot: {robot.name} with id {robot.robot_config['robot_id']}")

                    else:
                        node.get_logger().error(
                            f"Failed to initialize robot: {robot.name} with id {robot.robot_config['robot_id']}")

                    del missing_robots[robot.name]

                else:
                    pass
                    node.get_logger().debug(
                        f"{robot.name} with id {robot.robot_config['robot_id']} not found, trying again...")
        return
...
def main(argv=sys.argv):
    ...
    print(f"Starting InOrbit fleet adapter...")
    ...
    node = rclpy.node.Node(f'inorbit_fleet_command_handle')

    node.get_logger().info(
        f"Fleet [{fleet_name}] Node started")
